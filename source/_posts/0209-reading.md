...
预测的结果是一个 $T\times K\times H\times W\times L$ ：每个关节在每一个时间点在各个位置（3D坐标）上的概率值

#### 数据集

##### FullBodyManipulation 

==> 10 h 的同步数据 主体 + 15种不同的物体

人体骨骼的motion sequence + 物体的Mesh （包含位移和旋转）

##### TRUMANS

人体骨骼的motion sequence + 环境的Mesh

---

这里`人体的表征`与 `物体和环境` 的表征还是存在差异的

接下来我们把 `人体的表征` 和 `物体和环境` 统一表征成为voxel
...
更多的关注的是手和物体的交互。不需要深度图 也不用相机位姿

人物并不发生位移，第一视角固定，中间需要借助手和物体的mask

输入是第三视角的视频，第一视角首帧，文本指令

两个阶段，先通过 第一视角和第三视角的视频，预测得到第一视角下的手和物体交互mask

然后根据这个mask，首帧ego-view，text prompt 得到第一视角下的视频
